...
type ModalDegatEditProps = {
    degatSelect: DegatType
}

const ModalDegatEdit:React.FunctionComponent<ModalDegatEditProps> = ({degatSelect}) => {

//   const [intitule, setIntitule] = useState<string>(degatSelect.intitule);
//   const [description, setDescription] = useState<string>(degatSelect.decription);
//   const [images, setImages] = useState<ImageDegat[]>(degatSelect.imageList);
const [intitule, setIntitule] = useState<string>("");
  const [description, setDescription] = useState<string>("");
  const [images, setImages] = useState<ImageDegat[]>([]);
  const [isDragging, setIsDragging] = useState<boolean>(false);
  const [id, setId] = useState<number>(0);
  const {modalDegatEditIsOpen, closeModalDegatEdit, addDegat, degatAddList, setAllDegat, degatSelectEdit} = useStorageApp();
    const [messageError, setMessageError] = useState<String>("");
  
  //const idDegat = degatSelect.id;
    
  useEffect(()=>{
    setIntitule(degatSelectEdit.intitule);
    setDescription(degatSelectEdit.decription);
    setImages(degatSelectEdit.imageList);
    setId(degatSelectEdit.id);
  }, [degatSelectEdit])
    console.log(degatSelectEdit);
  const degatAddPress = ()=>{
    ...
    const degatFilter = degatAddList.map((item)=> item.id == id ? degatTemp : item);
    setAllDegat(degatFilter);
    //addDegat(degatTemp);
    setImages([]);
    setIntitule('');
    setDescription('');
            ...
            <div className='w-full h-full flex justify-center items-center z-50'>
                ...
                className='w-150 h-140 bg-white mt-10 pl-4.5 pr-4.5 z-5 rounded-lg overflow-y-scroll relative' style={{scrollbarWidth: 'none'}}>
                    <div className="mb-6 mt-6">
                        <h1 className="text-3xl font-bold text-gray-900 mb-2">Modifier un dégat</h1>
                        ...
                        {!intitule || !description || images.length == 0 ?
                                    <div className='flex justify-start mr-2.5' onClick={()=> setMessageError('Veillez remplir tous les champs et selectionner au moins une image')}>
                                        <div className={`flex justify-center py-3 rounded-lg my-8 cursor-pointer items-center gap-2 px-4 bg-gray-600 opacity-50`}>
                                            <h1 className={`${'text-white'} text-lg select-none font-normal inline-block`}>Modifier</h1>
                                        </div>
                                    </div>:
                            <ButtonDegatAdd title='Modifier' onPress={degatAddPress}/>
                        }
                    </div>
...
// import React, { useEffect, useState } from 'react'
// import InputFormSinistre from './input_form_sinistre';
// import upload from '../../../../core/resource/assets/upload.png';
// import drop from "../../../../core/resource/assets/delete.png";
// import ButtonDegatAdd from './button_degat_add';
// import { useStorageApp, type DegatType, type ImageDegat } from '../../../../core/resource/store/storageApp';
// import { AnimatePresence, motion } from 'framer-motion';
...
// const ModalDegatEdit:React.FunctionComponent = ({}) => {
...
//             <div className='w-full h-full flex justify-center items-center z-50'>
//                 <div className='w-full h-full bg-gray-950 opacity-20 absolute z-1' onClick={closeModalDegatEdit}/>

//                 <motion.div 
//                 initial={{scale: 1.1, opacity: .8}}
//                 animate={{scale: 1, opacity: 1}}
//                 exit={{scale: 1.1, opacity: .8}}
//                 className='w-150 h-140 bg-white mt-10 pl-4.5 pr-4.5 z-5 rounded-lg overflow-y-scroll' style={{scrollbarWidth: 'none'}}>
//                     <div className="mb-6 mt-6">
//                         <h1 className="text-3xl font-bold text-gray-900 mb-2">Modifier un dégat</h1>
//                         <p className="text-gray-600">Remplissez les détails du dégat</p>
//                     </div>
...
//                     </div>
//                     <div className='flex justify-end'>
//                         <ButtonDegatAdd onPress={closeModalDegatEdit} title='Annuler' type="outiline"/>
